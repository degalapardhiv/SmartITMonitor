...
def test_devices_returns_list(client, auth_headers):
    ...
    assert any(device["hostname"] == "kali" for device in devices)
...
def test_get_device_1_returns_kali(client, auth_headers):
    response = client.get("/devices/1", headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["hostname"] == "kali"
...
def test_device_metrics_history(client, auth_headers):
    response = client.get("/devices/1/metrics", headers=auth_headers)
    assert response.status_code == 200
